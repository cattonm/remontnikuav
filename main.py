...
_PRICES_CACHE_TTL = 300  # 5 хвилин
...
def _update_row_sync(row_id, data):
    ...
    try:
        ...
        address_full = f"{c.get('address')} ({c.get('area', '0')} м² | Пов: {c.get('floor', '1')} | Ліфт: {c.get('elevator', 'Немає')})"
        # Отримуємо поточний рядок, щоб зберегти статус
...
def _get_row_data_sync(row_id):
    ...
    if sheet:
        try:
            ...
            return row if len(row) >= 6 else None
        except:
            return None
    return None
...
def _delete_row_sync(row_id, user_name):
    try:
        ...
        if row_data:
            # Змінюємо статус на "видалена"
            if len(row_data) < 8:
                row_data.append("видалена")
            else:
                row_data[7] = "видалена"
            cell_list = sheet.range(f'A{row_id}:H{row_id}')
            ...
            # Копіюємо в аркуш "Кошик"
            try:
                trash_ws = doc.worksheet("Кошик")
            except:
...
def _save_report_sync(row_id, text):
    ...
    if sheet:
        try:
            sheet.update_cell(row_id, 7, text)  # стовпець G (індекс 7)
        except Exception as e:
            logging.error(f"Save report error: {e}")
...
def _get_orders_keyboard_sync(page=1):
    ...
    try:
        ...
        # Фільтруємо заголовок та активні рядки (статус "активна" або відсутній)
        active_rows = []
        for i, row in enumerate(all_rows[1:], start=2):  # i - номер рядка в таблиці
            status = row[7] if len(row) > 7 else "активна"
            if status == "активна":
                active_rows.append((i, row))  # зберігаємо номер рядка і дані
...
# ---------- Фонова задача очищення блокувань ----------
async def clean_locks_periodically():
    while True:
        ...
        for rid in expired:
            del _LOCKS[rid]
        if expired:
            logging.info(f"Cleaned {len(expired)} expired locks")
...
async def api_get_order(request):
    ...
    # Оновлюємо або встановлюємо блокування
...
async def api_live_calc(request):
    try:
        ...
        logging.error(f"Live calc error: {e}")
        return web.json_response({"error": str(e)}, status=500)
...
async def cmd_backup(message: Message):
    ...
    if csv_data:
        await message.answer_document(
            BufferedInputFile(csv_data, filename=f"remont_{datetime.now().strftime('%Y_%m_%d_%H%M')}.csv")
        )
...
async def change_page(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "📂 **Список заявок:**",
        reply_markup=await async_get_orders_keyboard(int(callback.data.split("_")[1]))
    )
...
async def show_first_page(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "📂 **Список заявок:**",
        reply_markup=await async_get_orders_keyboard(1)
    )
...
async def generate_report_action(callback: CallbackQuery):
    if is_throttled(callback.from_user.id, "generate_tz", delay=12):
        return await callback.answer("⏳ Зачекайте 12 секунд!", show_alert=True)
    ...
    try:
        ...
        for attempt in range(3):
            try:
                response = await model.generate_content_async(
                    GEMINI_PROMPT.format(raw_answers=raw_answers)
                )
        ...
        await callback.message.answer(f"❌ Помилка генерації ТЗ: {str(e)}")
    await callback.answer()
...
async def run_calculation(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer(f"❌ Помилка розрахунку: {str(e)}")

@dp.callback_query(F.data.startswith("del_"))
async def delete_order(callback: CallbackQuery):
    row_id = int(callback.data.split("_")[1])
    await async_delete_row(row_id, callback.from_user.full_name)
    await callback.answer("✅ Заявку переміщено в Кошик!", show_alert=True)
    await callback.message.edit_text(
        "📂 **Список заявок:**",
        reply_markup=await async_get_orders_keyboard(1)
    )
...
async def web_app_data_handler(message: Message):
    ...
    if await async_save_to_sheet(data):
        ...
        await message.answer("⚠️ Помилка збереження в Google Sheets.")

# ---------- Запуск ----------
async def on_startup(bot: Bot):
    # Перевіряємо доступ до Google Sheets
    sheet = _get_google_sheet()
    if not sheet:
        logging.error("Google Sheets недоступний! Перевірте GOOGLE_CREDS_JSON")
    else:
        logging.info("Google Sheets підключено успішно")
    
    try:
        await bot.set_webhook(f"{WEBHOOK_URL}{WEBHOOK_PATH}", secret_token=WEBHOOK_SECRET)
    except Exception as e:
        logging.error(f"Webhook setting error: {e}")
    
    # Запускаємо фоновий очищувач блокувань
