...
def validate_telegram_data(init_data: str, bot_token: str):
    try:
        ...
        if calc_hash == hash_val: return json.loads(parsed_data.get('user', '{}')).get('id')
        return None
    except: return None
...
async def async_log_action(user_name, action): asyncio.create_task(asyncio.to_thread(_log_action_sync, user_name, action))
...
# ==========================================
# ВІРТУАЛЬНІ ПЛОЩІ ДЛЯ "ЖИВОГО" КОШИКА
# ==========================================
...
async def api_save_order(request):
    ...
    try:
        ...
        if edit_id and str(edit_id) in _LOCKS: del _LOCKS[str(edit_id)]
...
# НОВИЙ API ДЛЯ ЖИВОГО КАЛЬКУЛЯТОРА
...
def get_main_menu_keyboard(): return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📝 Заповнити анкету", web_app=WebAppInfo(url=WEBAPP_URL))], [KeyboardButton(text="🔐 Кабінет менеджера")]], resize_keyboard=True)
...
async def cmd_reload_cache(message: Message):
    if message.from_user.id != MASTER_ADMIN_ID: return
    clear_auth_cache(); global _PRICES_CACHE; _PRICES_CACHE = None
...
# ОБРОБНИК ДЛЯ НОВИХ ЗАЯВОК (Вирішує проблему збереження!)
...
def main():
    ...
    # Реєстрація нового шляху для Live Calc
