"""Авторизація через Google Sheets (аркуші «Admins» і «Invites»).

ІСТОРИЧНА РЕАЛІЗАЦІЯ. Раніше цей файл називався security.py і був єдиним
джерелом правди про користувачів. Тепер це один із двох бекендів:
security.py став фасадом, який обирає між цим модулем і security_postgres.

Слабке місце, через яке і робився переїзд: усе тут залежить від живого
ключа сервіс-акаунта Google. Ключ помирає — get_all_authorized_users()
віддає порожній словник, і в кабінет не заходить ніхто, крім майстер-адміна.
"""
import os
import json
import logging
...
# Майстер-адмін працює навіть коли база/таблиця недоступні — це аварійний
# вхід. Значення винесено в оточення; фолбек лишений, щоб оновлення коду без
# оновлення env не замкнуло власника поза кабінетом.
MASTER_ADMIN_ID = int(os.getenv("MASTER_ADMIN_ID") or 845232133)
